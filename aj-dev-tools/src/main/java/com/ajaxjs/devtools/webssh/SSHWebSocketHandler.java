...
import com.ajaxjs.devtools.webssh.service.SSHConnectionManager;
...
public class SSHWebSocketHandler extends TextWebSocketHandler {
    ...
    private void startSSHChannel(WebSocketSession session, ChannelShell channel) {
        try {
            channel.connect();
            InputStream in = channel.getInputStream();// 处理SSH输出

            new Thread(() -> {  // 在单独的线程中读取SSH输出
    ...
    private void handleDisconnect(WebSocketSession session) {
        ...
        sessionLocks.remove(session); // 清理锁资源
    }
    ...
    private String getUserFromSession(WebSocketSession session) {
        return "anonymous";  // 简化实现，实际应用中可以从session中获取认证用户信息
    }
    ...
    private String getHostFromSession(WebSocketSession session) {
        return "unknown";  // 简化实现，实际应用中可以保存连接信息
    }
}